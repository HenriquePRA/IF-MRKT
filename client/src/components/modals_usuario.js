...
const ModalsUsuario = (props) => {
    ...
    const postMensagem = (event) => {
        ...
        if (titulo === "") {
            ...
            axios.post("/adicionar/mensagem", { destinatario: usrDestino, titulo: titulo, mensagem: mensagem }, config)
            .then((response) => {
    ...
    const submitAddContato = (event) => {
        ...
        if (email === "") {
            ...
            axios.post("/adicionar/contato", {"email": email}, config)
            .then((response) => {
                if (response.status === 200) {                    
                    setTimeout(() => {
    ...
    const submitAlterNome = (e) => {
        ...
        if (NovoNome === '') {
            ...
            axios.post("/alterar/nome", {nome: NovoNome}, config)
            .then((response) => {
                if (response.status === 200) {                    
                    setTimeout(() => {
    ...
    const submitAlterEmail = (e) => {
        ...
        if (NovoEmail === '') {
            ...
            axios.post("/alterar/email", {email: NovoEmail}, config)
            .then((response) => {
                if (response.status === 200) {
                    setTimeout(() => {
    ...
    const submitAlterBairro = (e) => {
        ...
        if (bairro === "") {
            ...
            axios.post("/alterar/bairro", { bairro: bairro }, config)
            .then((response) => {
                if (response.status === 200) {
                    setTimeout(() => {
    ...
    const submitAlterEndereco = (e) => {
        ...
        if (endereco === "") {
            ...
            axios.post("/alterar/endereco", { endereco: endereco }, config)
            .then((response) => {
                if (response.status === 200) {
                    setTimeout(() => {
    ...
    const disociarContatos = () => {        
        ...
        axios.post("/remover/contatos", { contatos: selectedContatos }, config)
        .then((response) => {
            gerarAlerta(response.data.status, "alert-success", "permanente")
            setTimeout(() => {
                setSelectedContatos([])
    ...
    const criarProduto = (e) => {
        ...
        if (nome === "") {
            ...
            axios.post("/adicionar/produto", { nome: nome, descricao: descricao, valor: valor, usado: usado, servico: servico, negociavel:negociavel }, config)
            .then((response) => {
                gerarAlerta(response.data.status, "alert-success", "permanente")
                setTimeout(() => {
                    props.setLoading(false)
